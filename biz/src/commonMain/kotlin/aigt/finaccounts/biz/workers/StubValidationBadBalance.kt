...
fun ICorChainDsl<FinAccountsContext>.stubValidationBadBalance(title: String) =
    worker {
        this.title = title
        on { stubCase == ContextStubCase.BAD_BALANCE && state == ContextState.RUNNING }
        handle {
            state = ContextState.FAILING
            this.errors.add(
                ContextError(
                    group = "validation",
                    code = "validation-balance",
                    field = "balance",
                    message = "Wrong balance field",
                ),
            )
        }
    }
